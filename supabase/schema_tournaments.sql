...
create table public.tournaments (
  ...
  creado_en timestamptz not null default now(),
  -- Migración 040: catálogo de estilos de bracket + fondos. Layout de
  -- cajas/líneas y fondo "galáctico" son independientes, se combinan.
  estilo_bracket text not null default 'clasico'
    check (estilo_bracket in ('clasico', 'esports', 'starcraft_oficial')),
  fondo_bracket text not null default 'ninguno'
  ...
  -- fase_actual arranca en 'eliminacion' para no romper el
  -- comportamiento de los torneos que no usan grupos -- se comportan
...
create table public.clan_wars (
  ...
  -- Migración 042: formato WTL/chino -- 3 sets Bo2 en posiciones fijas,
  -- con un ACE si el marcador global de mapas queda 3-3. 'simple'
  -- (el que ya existía) sigue siendo el default.
  formato text not null default 'simple' check (formato in ('simple', 'wtl')),
  ace_challenger_id uuid references public.profiles (id),
  ace_challenged_id uuid references public.profiles (id),
  -- No estaba en el pedido original, pero hace falta: sin esto
  -- cerrar_clan_war() no tendría dónde leer "el resultado del ACE".
  ace_ganador_id uuid references public.profiles (id),
  resultado_mapas_challenger integer not null default 0,
  resultado_mapas_challenged integer not null default 0,
...
-- proyecto.
-- Migración 043: p_formato -- sin esto no había ninguna forma de
...
-- ------------------------------------------------------------
-- Reprogramar una Clan War ya aceptada (migración 045): en vez de
-- solo poder abandonarla. reprogramaciones_usadas cuenta solo las
-- ACEPTADAS -- una rechazada no gasta el límite de 2.
-- ------------------------------------------------------------
...
create table public.clan_war_lineup (
  ...
  created_at timestamptz not null default now(),
  -- Migración 042: orden del jugador dentro del lineup (1/2/3) --
  -- solo tiene sentido en formato WTL, nullable para 'simple'. Varios
  -- NULL no chocan entre sí contra el unique de abajo.
  posicion integer check (posicion is null or posicion in (1, 2, 3)),
  check (
  ...
  unique (clan_war_id, team_id, posicion)
);
...
-- Se generan solos desde intentar_iniciar_clan_war() (más abajo),
-- emparejando la posición N del lineup de un lado contra la posición N
-- del otro -- por eso el lineup en formato WTL exige jugadores reales
-- (no temporales): este esquema no tiene un par jugador_id/
-- jugador_temporal_id como clan_war_lineup, así que no hay dónde
-- guardar un jugador temporal acá.
-- ------------------------------------------------------------
...
-- challenged quién dio el visto bueno de verdad (migración 038). En
-- formato WTL (migración 042), además exige que el lado que confirma
-- tenga exactamente 3 jugadores, en las 3 posiciones.
...
-- ------------------------------------------------------------
-- Formato WTL (migración 042): reportar_mapa_wtl() resuelve un mapa a
-- la vez dentro de un set Bo2 real -- los dos mapas siempre se juegan
-- (2-0 o 1-1), nunca corta en 1. designar_ace_wtl()/
-- reportar_mapa_ace_wtl() solo se pueden usar cuando el marcador
-- global de mapas quedó exactamente 3-3 con los 3 sets jugados.
-- ------------------------------------------------------------
...
-- la guerra está 'en_curso' (recién ahí terminó el check-in). Formato
-- 'simple' solamente -- WTL usa clan_war_wtl_sets + reportar_mapa_wtl().
...
begin
  ...
  -- Migración 042: en formato WTL, antes de dejar confirmar el
  -- cierre, exige que los 3 sets estén jugados y que, si hubo empate
  -- 3-3, el ACE ya tenga resultado.
  if v_reto.formato = 'wtl' then
    if exists (
      select 1 from public.clan_war_wtl_sets where clan_war_id = p_clan_war_id and status <> 'jugado'
    ) then
  ...
  if v_reto.formato = 'wtl' then
    ...
    -- empate 3-3) -- nunca queda 'empatada', el ACE siempre desempata.
    if v_reto.resultado_mapas_challenger = v_reto.resultado_mapas_challenged then
...
-- ------------------------------------------------------------
-- Overlay público para OBS (migración 044): clan_wars/
-- clan_war_wtl_sets/clan_war_matches son privadas por diseño (solo
-- dueño o capitán de alguno de los dos equipos) -- esta función arma
-- a mano un jsonb con solo lo que hace falta para un marcador en
-- vivo, sin exponer motivo_rechazo, caster_link, link_verificacion,
-- agregado_por ni ningún otro dato que no sea directamente el
-- marcador. El overlay de torneo no necesita una función equivalente:
-- tournaments/bracket_matches/tournament_participants/teams ya son
-- públicas.
-- ------------------------------------------------------------
...
-- grupo, antes de la llave eliminatoria. Mismo espíritu público de
-- lectura que bracket_matches -- un torneo se ve completo con el
-- link, grupos incluidos.
-- ------------------------------------------------------------
...
-- reportar_resultado_grupo(): mismo criterio de PERMISO que
-- reportar_resultado() (organizador o cualquiera de los dos
-- participantes), pero sin doble confirmación ni estado de disputa --
-- tournament_group_matches solo tiene 'pendiente'/'jugado', así que
-- cualquiera de los autorizados resuelve el partido al toque.
...
-- posiciones_grupos(): tabla de posiciones de todos los grupos de un
-- torneo de una sola vez (el frontend agrupa por group_id). Ordenada
-- por partidos ganados; empate por orden de inscripción -- sin
-- diferencia de mapas todavía (queda para cuando se construya el
-- formato WTL).
...
-- generar_llave(): mismo nombre y firma de siempre (ver el comentario
-- adentro) -- ahora también sirve para cerrar la etapa de grupos, no
-- se creó una función aparte para eso.
...
-- on delete cascade desde teams), después los equipos (que sí arrastra
